...
export default function Mentordashboard({ outpasses,actorName}: dashboardProps) {
  ...
  // FIXED: Track the specific ID instead of a boolean to avoid multi-modal triggering
    ...
    <div className="min-h-screen bg-slate-50 text-slate-800 p-4 md:p-8">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
          ...
          {(outPassIds.length === 0 || outPassIds.length === outpasses.length) && (
            <button
            type="button"
            disabled={bulkLoading || outpasses.length === 0}
            onClick={() => {outPassIds.length === outpasses.length?setOupassIds([]):setOupassIds(outpasses.map((pass)=>pass.id))}}
          ...
          >
            {bulkLoading ? (
              ...
              <>
                ...
                {outPassIds.length === outpasses.length? "Deselect All":"Select All"}({outpasses.length})
              </>
            )}
          </button>
          ...
          {(outPassIds.length > 0) && (
            <div className="flex items-center gap-3 animate-in fade-in slide-in-from-top-2 duration-200 justify-between w-full">
              <span className="text-sm font-medium text-slate-600 bg-slate-100 px-3 py-1.5 rounded-lg">
                {outPassIds.length} Selected
              </span>
              <button
                type="button"
                disabled={bulkLoading}
                onClick={() => handleBulkDecision("APPROVED")}
                className="px-4 py-2 text-white bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 rounded-xl font-medium text-sm transition-all shadow-sm"
              >
            ...
            {outpasses.length === 0 ? (
              ...
              outpasses.map((pass, idx) => {
                ...
                return (
                  <div
                    key={pass.id}
                    className={`group bg-white rounded-2xl border transition-all duration-200 shadow-sm hover:shadow-md ${
                    ...
                    <div className="p-4 md:p-5 flex flex-col sm:flex-row items-start gap-4">
                      ...
                      >
                        <div className="flex flex-wrap items-baseline gap-x-3">
                          ...
                          <span className="text-xs text-slate-400 font-mono">PASS ID: {pass.id.slice(0, 8)}</span>
                        </div>
